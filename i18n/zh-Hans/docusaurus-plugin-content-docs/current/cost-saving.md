...
专家模式是 Omx 中最有效的省钱功能之一。它使用智能体工具而不是基础工具来优化请求次数和上下文质量。
...
部分模型会输出详细的思考块，展示推理过程。这有助于透明度，但会增加上下文长度。

### 影响

在长时间对话中，思考块会占用大量上下文。裁剪它们可以：

- 减少每次请求的 token 消耗
- 保持对话专注于结果
- 在需要压缩之前为更多消息保持上下文

启用上下文编辑后，Omx 会自动裁剪旧消息里的思考块。
