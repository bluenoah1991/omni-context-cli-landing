...
| **Google Imagen** | 通过 Gemini generateContent 输出图像，用于图像生成 |
| **OpenAI Images** | 通过 OpenAI Images API（生成和编辑）进行图像生成 |
| **xAI Images** | 通过 xAI 图像 API 在聊天中生成图像 |

Image Workshop 还支持通过 Google Veo 和 xAI Video 进行视频生成。这些供应商的配置方式与其他模型相同，但会在 Workshop UI 中使用，而非通过聊天的 `ImageGen` 工具。

各供应商还有 **拦截器**，在不破坏原生协议的前提下处理特殊路由与兼容问题，例如 Anthropic 新旧模型、**CPA 代理会话亲和**、DeepSeek、Gemini 旧版、Kimi、MiniMax、OpenCode Go、通用 OpenAI 兼容端点、xAI、智谱 GLM 和 Zenmux 等。
...
| **Provider** | `openai`、`anthropic`、`gemini`、`responses`、`google-imagen`、`openai-imagen`、`xai-image`、`google-video` 或 `xai-video` |
| **API Key** | 对应端点使用的 key |
...
**图像生成模型** 在 Web 设置或应用配置里单独选择（`defaultImageGenModelId`）。只有配置了可用图像模型时，允许 `ImageGen` 的工作流才会向模型暴露该工具——可设为具体模型 id，未设置则使用列表中第一个 `google-imagen`、`openai-imagen` 或 `xai-image` 条目。将 `defaultImageGenModelId` 设为 `disabled` 可关闭聊天内图像生成，即使列表里仍有图像模型。
