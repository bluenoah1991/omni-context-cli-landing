...
| 工作流 | 使用场景 | 描述 |
|--------|----------|------|
| **Programming** | 终端、VS Code | 默认的编程工作流。包含一个精选的内置工具子集（文件操作、搜索、atlas、skill、web），外加内置智能体工具、自定义 Agent、MCP 和远程工具。 |
...
如果你使用桌面应用，它还会额外安装三个工作流：

- **Browser** - 给 Chrome 侧边栏使用，带有标签页、页面、书签、历史记录、截图以及受限的页面脚本执行能力。
- **Memo** - 用于个人提醒和通知。在 macOS 上，备忘事项会通过原生桥接直接同步进系统的“提醒事项”；在其他平台上则回退到本地 JSON 文件存储。
- **Research** - 用于 PDF 密集型研究和分析，支持 PDF 拆分、分块阅读、预览和网页搜索。
