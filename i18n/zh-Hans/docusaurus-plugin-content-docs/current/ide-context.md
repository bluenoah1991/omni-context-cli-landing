...
该扩展提供了两种在编辑器中与 OmniContext CLI 交互的方式：

- **侧边面板** — 点击活动栏中的 OmniContext 图标，在侧边栏打开聊天面板
- **编辑器标签页** — 运行 `OmniContext: Open in Editor` 命令，在完整的编辑器标签页中打开聊天

两个入口连接到同一个 OmniContext CLI 实例，你的会话、模型和记忆都是共享的。

扩展在激活时还会**实时广播你的当前选区**。如果 IDE 上下文已启用，助手会随着光标移动实时看到你高亮的内容——无需复制粘贴文件路径或代码片段。
