...
Omx 支持为 Web UI 设置密码验证。当需要将服务器暴露到网络或部署到公有云时，这是必不可少的安全措施。
...
系统会以交互方式提示你输入密码。你也可以直接传入：

```bash
omx --set-password my-secret-password
```

密码使用 `scrypt` 算法加密后存储在 `~/.omx/password`，明文密码不会被保存。

### 工作原理

设置密码后，Web UI 在授权访问前会显示登录页面。其底层机制如下：

1. 你在浏览器中输入密码
2. 服务器验证密码后签发一个 Bearer Token
3. Token 存储在浏览器的 localStorage 中，并随每个请求发送
4. Token 有效期为 30 天

### 频率限制

为防止暴力破解，服务器会在 5 次登录失败后锁定该 IP。锁定时间采用指数退避策略，起始锁定时间为 30 秒。
...
- **应用集成** - 通过专门的设置标签页连接 Office、浏览器和 Figma
- **自定义提示词** - 为专家模式、艺术家模式、探索者模式和助理模式自定义系统提示词
