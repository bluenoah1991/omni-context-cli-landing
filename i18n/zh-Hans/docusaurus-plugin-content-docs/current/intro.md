...
OmniContext CLI 是一个上下文优先、零遥测的助手，起点在终端，也能延伸到 VS Code、Office、浏览器和移动端。它原生支持 Anthropic、OpenAI、Gemini 和 OpenAI Responses API，以及通过 Google Imagen、OpenAI Images 和 xAI Images 进行图像生成，Image Workshop 中还可通过 Google Veo 和 xAI Video 进行视频生成，并内置工作流、智能体工具、Atlas 导航和跨会话记忆。
...
- `.omx/tools/`、`.omx/model-prompts/`、`.omx/hooks.json` - 外部工具、模型提示与 hooks（详见项目文件文档）
...
| `-i, --image-workshop` | 启动服务器后在浏览器中打开 Image Workshop |
| `-p, --port <port>` | 服务器模式使用的端口（默认 5281） |
| `-H, --host <host>` | 服务器模式使用的主机（默认 localhost） |
