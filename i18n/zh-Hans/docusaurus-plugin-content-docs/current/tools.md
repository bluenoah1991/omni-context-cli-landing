...
| **ArtifactList** | 列出当前会话中由模型生成的所有 artifacts 及其文件路径。 |
| **Bash** | 运行 shell 命令，用于构建、安装、测试和其他终端任务。支持后台任务。 |
| **BashOutput** | 通过任务 ID 查看后台 Bash 任务的输出。 |
| **Edit** | 对文件进行精确的原地文本替换。 |
| **Glob** | 按模式查找文件，并支持 `.gitignore`。 |
| **Grep** | 用正则和行号搜索文件内容。 |
| **AtlasDelete** | 从 Atlas 导航系统中删除一个地图。 |
| **AtlasList** | 列出当前项目里已保存的 Atlas 地图。 |
| **AtlasRead** | 读取某个 Atlas 地图，包括它关联的文件、目录和符号。 |
| **AtlasUpdate** | 创建或更新 Atlas 地图，让有价值的代码导航知识能跨会话保留下来。 |
| **ImageGen** | 通过 Google Imagen、OpenAI Images 或 xAI Images 生成图像并保存为会话 artifact（需已配置图像模型）。 |
...
| **WebSearch** | 搜索网页上的最新信息。Agent 模型池里包含 Anthropic、Responses 或 Gemini 模型时可用。 |
| **Write** | 从头写入文件或整文件覆盖。支持 `createOnly`，让新建文件更安全。 |

Atlas 更适合被看成一套独立的导航子系统，而不是一个零散小功能。想看完整说明，可以直接跳到 [Atlas 导航](/tutorial/atlas)。

:::note Programming 工作流的工具白名单

默认的 **Programming** 工作流只暴露一个精选的内置工具子集：`Bash`、`BashOutput`、`Edit`、`Glob`、`Grep`、`Read`、`Write`、`WebFetch`、`WebSearch`、`Skill` 和四个 atlas 工具。它**不会**默认包含 `MemoryList`/`MemoryUpdate`/`MemoryDelete`、`RecallSearch`/`RecallRead`、`ImageGen`、`ArtifactList` 或 `AnalyticsQuery`。自定义工作流、**General** 预设以及单独的 **Memory**/**Recall**/**Analytics** 工作流则有各自不同的工具选择。
...
| **Agent_Audit** | 对指定代码区域进行正确性、架构、数据流和质量审计，并给出结构化发现。 |
...
`--approve-all` 也会为 MCP 和远程工具调用弹确认。智能体工具本身不会直接弹确认，但它们内部调用的内置工具仍然会遵守当前审批模式。

### 并行执行

OmniContext CLI 可以同时运行多个只读或彼此独立的工具调用。当它检测到几个工具调用互不依赖时——比如并行读取多个文件，或者同时执行几个独立的搜索——就会并发派发它们。智能体工具在任务独立时也能并行运行。这样当模型一次发出多个工具调用时，往返延迟会显著降低。
